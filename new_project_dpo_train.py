...
class MyDPOTrainer(DPOTrainer):
    def __init__(self, custom_train_loader=None, retain_dataloader=None, *args, **kwargs):
    ...
    def compute_preserve_loss(self, model, preserve_batch):
        # Compute loss for preserve data
        out = model(**preserve_batch)
        return out.loss

    def get_train_dataloader(self):
        # Use custom train_dataloader if provided
        if self.custom_train_loader is not None:
            return self.custom_train_loader
        # Otherwise use parent class default logic
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Tokenize prompt / chosen / rejected separately
        ...
        # Call parent class's compute_loss to get dpo_loss
...
def main():
    ...
    parser.add_argument("--pref_num", type=int, default=2, help="Number of preference samples per batch.")
    ...
    trainer = MyDPOTrainer(
        ...
        retain_dataloader=train_loader
    )
